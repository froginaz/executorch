...
def get_16bit_sigmoid_quantizer(tosa_str: str):
    ...
    quantizer.set_io(_get_16_bit_quant_config())
...
def test_sigmoid_tosa_u55_on_fvp(test_data):
    pipeline = EthosU55PipelineBI(
...
@common.SkipIfNoCorstone300
def test_sigmoid_add_sigmoid_tosa_u55_on_fvp(test_data):
    pipeline = EthosU55PipelineBI(
        SigmoidAddSigmoid(),
        test_data,
        Sigmoid.aten_op,
...
def test_sigmoid_tosa_u85_on_fvp(test_data):
    pipeline = EthosU85PipelineBI(
        Sigmoid(), test_data, Sigmoid.aten_op, Sigmoid.exir_op, run_on_fvp=True
    )
    pipeline.change_args("quantize", get_16bit_sigmoid_quantizer("TOSA-0.80+BI"))
...
@common.SkipIfNoCorstone320
def test_sigmoid_add_sigmoid_tosa_u85_on_fvp(test_data):
    pipeline = EthosU85PipelineBI(
        SigmoidAddSigmoid(),
        test_data,
        Sigmoid.aten_op,
